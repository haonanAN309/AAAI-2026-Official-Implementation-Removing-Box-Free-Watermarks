...
class Inception(nn.Module):
    def __init__(self, in_channels=32):
        ...
        self.concat_conv1x1 = nn.Conv2d(3 * in_channels, in_channels, kernel_size=1)
    ...
    def forward(self, x):
        ...
        return residual + concatenated

class EA(nn.Module):
    def __init__(self, in_channels=3):  # 修改为支持RGB输入
        super(EA, self).__init__()
        self.conv1 = nn.Conv2d(in_channels, 32, kernel_size=3, padding=1)
        self.inception1 = Inception(in_channels=32)
        self.inception2 = Inception(in_channels=32)
        self.conv2 = nn.Conv2d(32, 3, kernel_size=3, padding=1)  # 输出3通道以匹配RGB
...
class EB(nn.Module):
    def __init__(self, in_channels=1):  # 修改为支持RGB输入
...
class EC(nn.Module):
    def __init__(self, in_channels=51):  # 3 (EA输出) + 48 (EB输出) = 51
        super(EC, self).__init__()
        self.conv1 = nn.Conv2d(in_channels, 48, kernel_size=3, padding=1)
        self.conv2 = nn.Conv2d(48, 24, kernel_size=3, padding=1)
        self.conv3 = nn.Conv2d(24, 3, kernel_size=3, padding=1)  # 输出3通道以匹配RGB
...
class HidingENet(nn.Module):
    def __init__(self):
        ...
        self.ea = EA(in_channels=3)  # RGB输入
        self.eb = EB(in_channels=1)  # RGB输入
        self.ec = EC(in_channels=51)  # 3 + 48 = 51
