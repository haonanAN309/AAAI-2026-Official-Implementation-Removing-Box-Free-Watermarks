...
parser.add_argument('--FHnet', default='',
                    help="path to SurrogateNet (to continue training)")
parser.add_argument('--trainpics', default='output_forward_HNet_RGB/',
                    help='folder to output training images')
parser.add_argument('--validationpics', default='output_forward_HNet_RGB/',
                    help='folder to output validation images')
parser.add_argument('--testPics', default='output_forward_HNet_RGB/',
                    help='folder to output test images')
parser.add_argument('--runfolder', default='output_forward_HNet_RGB/',
                    help='folder to output test images')
parser.add_argument('--outckpts', default='output_forward_HNet_RGB/',
                    help='folder to output checkpoints')
parser.add_argument('--outlogs', default='output_forward_HNet_RGB/',
                    help='folder to output images')
parser.add_argument('--outcodes', default='output_forward_HNet_RGB/',
...
def main():
    ...
    global opt, optimizerFHnet, writer, logPath, schedulerFHnet, val_loader
    ...
    FHnet = UnetGenerator(input_nc=3, output_nc=3, num_downs= opt.num_downs, output_function=nn.Sigmoid)
    FHnet.cuda()
    FHnet.apply(weights_init)

    Enet = HidingRes(in_c=3, out_c=3)
    Enet.cuda()

    # setup optimizer
    optimizerFHnet = optim.Adam(FHnet.parameters(), lr=opt.lr, betas=(opt.beta1, 0.999))
    schedulerFHnet = ReduceLROnPlateau(optimizerFHnet, mode='min', factor=0.2, patience=5, verbose=True)
    ...
    if opt.FHnet != '':
        FHnet.load_state_dict(torch.load(opt.FHnet))
    if opt.ngpu > 1:
        FHnet = torch.nn.DataParallel(FHnet).cuda()
    print_network(FHnet)
    ...
    for epoch in range(opt.niter):
        ######################## train ##########################################
        train(train_loader, epoch, Hnet=Hnet, FHnet=FHnet, Enet=Enet)

        ####################### validation  #####################################
        val_sumloss= validation(val_loader,  epoch, Hnet=Hnet, FHnet=FHnet,  Enet=Enet)

        ####################### adjust learning rate ############################
        schedulerFHnet.step(val_sumloss)

        # save the best model parameters
        if val_sumloss < globals()["smallestLoss"]:
            globals()["smallestLoss"] = val_sumloss
            # for parallel training: FHnet.module.state_dict()
            torch.save(FHnet.module.state_dict(),
                       '%s/FHnet_epoch_%d,sumloss=%.6f.pth' % (
                           opt.outckpts, epoch, val_sumloss))
    writer.close()


def train(train_loader, epoch, Hnet, FHnet, Enet):
    batch_time = AverageMeter()
    data_time = AverageMeter()
    FHnetlosses = AverageMeter()
    ...
    for i, data in enumerate(train_loader, 0):
        ...
        generated_image = FHnet(cover_img_Bv)
        uncover_img_B = Enet(cover_img_B)
        uncover_img = Enet(containerv)
        uncover_img_generated = Enet(generated_image)

        err_FHnet = opt.betamse * mse_loss(generated_image, containerv)

        err_FHnet.backward()
        optimizerFHnet.step()

        FHnetlosses.update(err_FHnet.data, this_batch_size)
        ...
        log = '[%d/%d][%d/%d]\t Loss_sum: %.4f \tdatatime: %.4f \tbatchtime: %.4f' % (
            epoch, opt.niter, i, len(train_loader),
               FHnetlosses.val, data_time.val, batch_time.val)
    ...
    epoch_log = "one epoch time is %.4f======================================================================" % (
        batch_time.sum) + "\n"
    epoch_log = epoch_log + "epoch learning rate: optimizerFHnet_lr = %.8f" % (
        optimizerFHnet.param_groups[0]['lr']) + "\n"
    epoch_log = epoch_log + "epoch_sumloss=%.6f" % (FHnetlosses.avg)

    print_log(epoch_log, logPath)

    writer.add_scalar("lr/H_lr", optimizerFHnet.param_groups[0]['lr'], epoch)
    writer.add_scalar("lr/beta", opt.beta, epoch)
    writer.add_scalar('train/Sum_loss', FHnetlosses.avg, epoch)


def validation(val_loader, epoch, Hnet, FHnet, Enet):
    print(
        "#################################################### validation begin ########################################################")
    start_time = time.time()
    Hnet.eval()
    ...
    val_log = "validation[%d] val_sumloss = %.6f\t validation time=%.2f" % (
        epoch, FHnetlosses.avg, val_time)

    print_log(val_log, logPath)

    writer.add_scalar('validation/Sum_loss', FHnetlosses.avg, epoch)

    print(
        "#################################################### validation end ########################################################")

    return FHnetlosses.avg
